...
export class ProfilerService {
  ...
  private readonly eventMap = new Map<string, EventProfile>();
  ...
  addCache(cacheProfile: import('../common/profiler.model').CacheProfile) {
    ...
    if (profile) {
      ...
      this.logger.debug(
        `Profiler: Skipping cache capture for ${cacheProfile.key} - No active request context (ALS is empty).`,
      );
    }
  }
  ...
  async getSummaryStats(): Promise<{
    ...
    const errorProfiles = profiles.filter(
      (p) => p.statusCode && p.statusCode >= 400,
    );
    ...
    const nPlusOneCount = allQueries.filter((q) =>
      q.tags?.includes('n+1'),
    ).length;
    const seqScanCount = allQueries.filter((q) =>
      q.tags?.includes('seq-scan'),
    ).length;
    ...
    const endpointMap = new Map<
      string,
      { durations: number[]; method: string }
    >();
    profiles.forEach((p) => {
      const key = `${p.method}:${p.route || p.url}`;
      if (!endpointMap.has(key))
        endpointMap.set(key, { durations: [], method: p.method });
    ...
    const topSlowEndpoints = Array.from(endpointMap.entries())
      .map(([key, val]) => ({
        ...
        method: val.method,
        avgDuration:
          val.durations.reduce((s, d) => s + d, 0) / val.durations.length,
    ...
    // Recent errors
    const recentErrors = errorProfiles.slice(0, 5).map((p) => ({
      id: p.id,
      url: p.url,
      method: p.method,
    ...
    const memoryUsage = latestWithMemory?.memory
      ? {
          ...
          heapTotal: Math.round(
            latestWithMemory.memory.heapTotal / 1024 / 1024,
          ),
        }
      : null;
  ...
  /** Store a new event profile (called by EventCollector on each emit) */
  addEvent(event: EventProfile): void {
    if (this.eventOrder.length >= MAX_EVENTS) {
      const oldest = this.eventOrder.shift();
      if (oldest) this.eventMap.delete(oldest);
    }
    this.eventMap.set(event.id, event);
    this.eventOrder.push(event.id);
  }

  /** Finalise an event after all its listeners have run */
  finalizeEvent(id: string, status: 'success' | 'error', error?: string): void {
    const ev = this.eventMap.get(id);
    if (!ev) return;
    ev.totalDuration = Date.now() - ev.emittedAt;
    ev.status = status;
    if (error) ev.error = error;
  }

  /** Append a listener trace to an in-flight event */
  addListenerTrace(
    eventId: string | undefined,
    trace: EventListenerTrace,
  ): void {
    if (!eventId) return;
    const ev = this.eventMap.get(eventId);
    if (ev) ev.listeners.push(trace);
  }

  /** Record a child event ID on the parent */
  linkChildEvent(parentId: string, childId: string): void {
    const parent = this.eventMap.get(parentId);
    if (parent && !parent.childEventIds.includes(childId)) {
      parent.childEventIds.push(childId);
    }
  }

  /** Return the current request ID (for linking events to requests) */
  getCurrentRequestId(): string | undefined {
    return this.als.getStore()?.id;
  }

  /** Return all captured events, newest first */
  getEventsList(): EventProfile[] {
    const all = [...this.eventMap.values()];
    all.sort((a, b) => b.emittedAt - a.emittedAt);
    return all;
  }

  /** Return root events (no parent) for tree rendering */
