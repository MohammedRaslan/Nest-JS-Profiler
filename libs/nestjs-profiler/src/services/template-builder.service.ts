...
export class TemplateBuilderService {
  ...
  /** Build event tracking page shell — all data fetched client-side via /__profiler/api/events */
  ...
  buildSummaryPage(
    stats: Awaited<
      ReturnType<
        import('./profiler.service').ProfilerService['getSummaryStats']
      >
    >,
  ): string {
    const errorCount = Math.round(
      (stats.totalRequests * stats.errorRate) / 100,
    );
    ...
    const slowQueriesTextClass =
      stats.slowQueries > 0 ? 'text-yellow-600' : 'text-gray-900';

    // N+1 styling
    const nPlusOneBadgeClass =
      stats.nPlusOneCount > 0 ? 'bg-orange-50' : 'bg-green-50';
    const nPlusOneIconClass =
      stats.nPlusOneCount > 0 ? 'text-orange-500' : 'text-green-600';
    const nPlusOneTextClass =
      stats.nPlusOneCount > 0 ? 'text-orange-600' : 'text-gray-900';
    ...
    const topQueriesTable =
      stats.topSlowQueries.length > 0
        ? `<div class="divide-y divide-gray-100">
                ${stats.topSlowQueries
                  .map(
                    (q) => `
                    <div class="px-5 py-3">
  ...
  buildDashboard(profiles: RequestProfile[]): string {
    ...
    const emptyState = !rows
      ? '<div class="p-8 text-center text-gray-500">No requests captured yet.</div>'
      : '';
  ...
  buildDetail(profile: RequestProfile): string {
    const queries =
      profile.queries.map((q, i) => this.buildQueryDetail(q, i)).join('') ||
  ...
  buildQueriesList(queries: any[]): string {
    const rows =
      queries
        .map((q) =>
          this.viewService.render('partials/query_row', {
            sql: q.sql,
        ...
        .join('') ||
      '<tr><td colspan="3" class="p-8 text-center text-gray-500">No queries found.</td></tr>';
  ...
  buildLogsList(
    ...
    const rows =
      logs.map((l) => this.buildLogRow(l)).join('') ||
  ...
  buildEntitiesList(entities: any[]): string {
    const rows = entities
      .map((e) =>
        this.viewService.render('partials/entity_row', {
          name: e.name,
    ...
    const emptyState = !rows
      ? '<div class="p-8 text-center text-gray-500">No entities found.</div>'
      : '';
  ...
  /**
   * Build routes list HTML — passes routes as embedded JSON for client-side accordion rendering.
   */
  buildRoutesList(routes: any[]): string {
    const safe = routes.map((r) => ({
      path:       r.path,
      method:     r.method,
      ...
      body: r.body ? {
        typeName:   r.body.typeName,
        properties: r.body.properties || [],
        file:       r.body.file || '',
      } : null,
    }));
    ...
    return this.viewService.render('routes', {
      ...
      routesJson:  JSON.stringify(safe),
    });
  }
  ...
  buildCacheList(cacheOps: any[]): string {
    const rows = cacheOps
      .map((c) =>
        this.viewService.render('partials/cache_row', {
    ...
    const emptyState = !rows
      ? '<div class="p-8 text-center text-gray-500">No cache operations recorded.</div>'
      : '';
  ...
  private buildRequestRow(p: RequestProfile): string {
    ...
    return this.viewService.render('partials/request_row', {
      ...
      queriesCount:
        p.queries.length > 0
          ? `<span class="bg-gray-100 text-gray-700 px-2 py-0.5 rounded-full text-xs">${p.queries.length}</span>`
  ...
  private buildQueryDetail(q: any, index: number): string {
    return this.viewService.render('partials/detail_query_row', {
      ...
      opBadge: q.operation
        ? `<span class="text-xs px-2 py-0.5 rounded bg-purple-100 text-purple-800 font-medium">${q.operation}</span>`
        : '',
      connection: q.connection
        ? `<span class="text-xs px-2 py-0.5 rounded border border-gray-200 text-gray-500 bg-gray-50">${q.connection}</span>`
        : '',
      ...
      tagsBadges: (q.tags || [])
        .map((t: string) => this.getTagBadge(t))
        .join(''),
      duplicationWarning:
        q.duplicatedCount > 1
          ? `<div class="mt-1 text-xs text-orange-600 font-medium flex items-center"><svg class="w-3 h-3 mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4m0 4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z"></path></svg> Executed ${q.duplicatedCount} times (N+1)</div>`
          : '',
      planType: q.planType
        ? `<span class="ml-2 text-xs text-gray-400">(${q.planType})</span>`
        : '',
      params:
        q.params && q.params.length
          ? `
  ...
  private buildMetadataSidebar(p: RequestProfile): string {
    return this.viewService.render('partials/metadata_sidebar', {
      statusColor:
        (p.statusCode || 200) >= 400 ? 'text-red-600' : 'text-green-600',
  ...
  private buildLogRow(l: any): string {
    return this.viewService.render('partials/log_row', {
      ...
      message: l.message,
      context: l.context
        ? `<span class="ml-2 text-xs text-gray-500">[${l.context}]</span>`
        : '',
  ...
  buildHttpCallsList(calls: any[]): string {
    const rows =
      calls
        .map((c) => {
          const statusClass = !c.statusCode
            ? 'text-gray-400'
            : c.statusCode >= 500
        ...
        .join('') ||
      '<tr><td colspan="5" class="p-8 text-center text-gray-500">No outbound HTTP calls recorded.</td></tr>';
